...
export default function Exercicio02({initialName = ''}) {
    ...
    const [name, setName] = React.useState(
        ...
        // ou usa o valor de initialName, caso o item do localStorage não exista

        // Retornando uma função para criar  um lazy initializer, que 
        // é necessário apenas uma vez, no carregamento inicial da página
    ...
    // Armazenando em um item do localStorage chamado "name" o valor da variável
    // de estado name
    React.useEffect (() => {
        window.localStorage.setItem('name', name) 
        setCount (count + 1)
    ...
    // 2) Vetor de depêndencia: qual ou quais as variáveis de estado
    // que serão monitoradas para a execução do useEffect ()
    
    function handleChange(event) {
        // Atualização da variável de estado "name" vai desencadear 
        // uma atualização do componente. Após a atualização ter sido processada,
        // será executado o useEffect ().
